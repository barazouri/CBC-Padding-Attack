...
var app = express();
app.use("/getChallenge", function (req, res) { return __awaiter(_this, void 0, void 0, function () {
    var _a, key, iv, data;
...
app.post("/attemptChallenge", bodyParser.json(), function (req, res) { return __awaiter(_this, void 0, void 0, function () {
...
function prepare(data) {
    ...
    Buffer.alloc(paddingSize, paddingSize).copy(prep, data.length + tag.length, 0);
    console.log(prep);
    return prep;
}
...
function dec(data, key, iv) {
    ...
    var padding = dec.slice(dec.length - dec[dec.length - 1], dec.length);
    if (padding[padding.length - 1] > 8)
    ...
    if (padding.length > 1) {
        console.log(padding);
        console.log("bad pad ok !");
    }
    var decNoPad = dec.slice(0, dec.length - padding[0]);
    ...
    console.log(decNoPad.toString('ascii'));
    return { error: 'none' };
}
